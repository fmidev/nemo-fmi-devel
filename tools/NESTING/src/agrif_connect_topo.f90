...
!             G0%Bathy_meter(ipt,jpt) = zmin
             G0%Bathy_meter(ipt,jpt) =& 
             SUM(G1%Bathy_meter(ji-irafx/2+diff:ji+irafx/2, &
                                 jj-irafy/2+diff:jj+irafy/2))&
                /irafx/irafy            
             IF(G0%Bathy_meter(ipt,jpt).GT.0.) THEN
                 G0%Bathy_meter(ipt,jpt) = MAX( G0%Bathy_meter(ipt,jpt),zmin)   
             ENDIF
             !
             compt = compt + 1
             !
