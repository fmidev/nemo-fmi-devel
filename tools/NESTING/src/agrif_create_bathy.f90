...
PROGRAM create_bathy
  ...
  REAL*8, DIMENSION(:,:),POINTER :: wtab  => NULL()
  ...
  IF(.NOT.new_topo .AND. .NOT.partial_steps) THEN      
     ...
     !
     boundary = connectionsize*irafx + nbghostcellsfine + 1 
     !
  ...
  ELSE
     ...
     IF( partial_steps ) THEN                
        ...
        connectionsize = 2 + nb_connection_pts 
        !            
        gdepw_ps_interp = 0.
        CALL Check_interp(G0,gdepw_ps_interp)      ! interpolation in connection zone (3 coarse grid cells)
        !
        ...
        !
        !
        IF (.NOT.ASSOCIATED(wtab)) &
             ALLOCATE(wtab(SIZE(G1%bathy_meter,1),SIZE(G1%bathy_meter,2)))
        wtab(:,:) = 0.
        wghts = 1.
        DO ji = boundary + 1 , boundary + nb_connection_pts * irafx
            wghts = wghts - (1. / (nb_connection_pts*irafx - 1. ) )
            DO jj=boundary+1,nyfin-boundary
               wtab(ji,jj) = MAX(wghts, wtab(ji,jj))  
            END DO
        END DO 
        ...
        DO jj = boundary + 1 , boundary + nb_connection_pts * irafy
            wghts = wghts - (1. / (nb_connection_pts*irafy - 1. ) )
            DO ji=boundary+1,nxfin-boundary
               wtab(ji,jj) = MAX(wghts, wtab(ji,jj))
            END DO
        END DO
        ...
        G1%gdepw_ps(:,:) = (1.-wtab(:,:)) * G1%gdepw_ps(:,:) + gdepw_ps_interp(:,:)*wtab(:,:)
        ...
        IF(smoothing) THEN 
           ...
           !
           boundary = (connectionsize+nb_connection_pts)*irafx + nbghostcellsfine + 1 
