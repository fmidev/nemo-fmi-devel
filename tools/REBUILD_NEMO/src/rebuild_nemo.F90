...
found_num_args=iargc()
!Check that the required argument is present, if it is not then set it to the default value: nam_rebuild
IF (found_num_args <= 0) THEN
   WRITE(numout,*) 'Namelist path not supplied as command line argument. Using default, nam_rebuild.'
   namelist_path='nam_rebuild'
ELSE IF (found_num_args == 1) THEN
   CALL getarg(1, namelist_path)
ELSE 
   WRITE(numerr,*) 'ERROR! : Incorrect number of command line arguments. Please supply only'
...
!1.2 Read in the namelist 
   ...
   nchunksize = 0
   deflate_level = 0
   OPEN( UNIT=numnam, FILE=TRIM(namelist_path), FORM='FORMATTED', STATUS='OLD' )
