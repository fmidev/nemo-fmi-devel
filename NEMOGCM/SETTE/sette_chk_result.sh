...
#
  if [ -d $vdir/$nam ]; then
    ...
    ./rebuild_nemo -t 4 ../../CONFIG/SAS_ST/LONG/SAS_00000240_restart_ice  $NPROC > /dev/null 2>&1
    #SF echo "REBUILD SHORT restart SAS files, without standard output"
    ./rebuild_nemo -t 4 ../../CONFIG/SAS_ST/SHORT/SAS_00000240_restart_ice $NPROC >&-
    cd ${SAS_RESTART_DIR}/LONG
...
#
  if [ -d $vdir/$dir1 ] && [ -d $vdir/$dir2 ]; then
    ...
    rep1=`ls -1rt $vdir/$dir1/$mach/$dorv1/ |  tail -2l | head -1 `
    rep2=`ls -1rt $vdir/$dir2/$mach/$dorv2/ |  tail -1l`
...
# Directory to run the tests
  NEMO_VALID_REF=/path/to/reference/sette/results
  NEMO_REV_REF=0000
...
#
cmd="svn"
[ ! -d "$SETTE_DIR/.svn" ] && cmd="git $cmd"
# define revision as in all_function
revision=`$cmd info ../ | grep -i "Last Changed Rev:" | sed -e "s/ //g" | cut -d ":" -f 2`
branchname=`$cmd info | grep URL | rev | cut -d "/" -f 3 | rev`
echo "SETTE validation report : $branchname @ r$revision"
#
...
# Restartability test
#
 for restart_test in WGYREPIS_ST WORCA2SI3PIS_ST WORCA2OFFPIS_ST WAMM12_ST WISOMIP_ST WORCA2AGR_ST
 do
...
#
 for repro_test in WGYREPIS_ST WORCA2SI3PIS_ST WORCA2OFFPIS_ST WAMM12_ST WISOMIP_ST WORCA2_SI3_OBS_ST WORCA2AGR_ST
 do
...
# AGRIF special check
#
 dir1=WORCA2AGUL_NOZOOM_ST
 dir2=WORCA2AGUL_NOAGR_ST
...
#
 if [ $NEMO_VALID_REF != "/path/to/reference/sette/results" ]; then
   ...
   echo "$NEMO_VALID at rev $revision"
   echo 'and'
   echo "$NEMO_VALID_REF at rev $NEMO_REV_REF"
   for repro_test in WGYREPIS_ST WORCA2SI3PIS_ST WORCA2OFFPIS_ST WAMM12_ST WISOMIP_ST WORCA2_SI3_OBS_ST WORCA2AGR_ST
   do
   ...
   echo ''
   echo 'Result are not compare with any other revision. To do it please fill NEMO_VALID_REF and NEMO_REV_REF in sette_rpt.sh'
   echo ''
 fi

done
#
## Additional checks for ORCA2AGUL/FIRST standalone AGRIF test
## still to be added
